...
class Ui_wechat_ai(QWidget):
    ...
    def showMessageBox(self):
        QMessageBox.information(self, 'Information', 'This is an information message.')  # 显示信息消息框
    ...
    def setupUi(self, wechat_ai):
        ...
        wechat_ai.resize(900, 600)  # 增大窗口尺寸
        wechat_ai.setStyleSheet(self.STYLE_SHEET)  # 应用样式表
        ...
        # 标题
        title_label = QLabel("微信AI客服助手")
        title_label.setObjectName("titleLabel")
        title_label.setAlignment(Qt.AlignCenter)
        left_layout.addWidget(title_label)

        # 第一组：基本设置（包含原基本设置和群聊设置）
        ...
        self.checkbox_isUse = QtWidgets.QCheckBox("启用知识库")
        self.checkbox_isMatch = QtWidgets.QCheckBox("未匹配不回复")
        self.checkbox_newMatch = QtWidgets.QCheckBox("最近消息检索")
        self.checkbox_aite = QtWidgets.QCheckBox("艾特后回复")
        # 新增群聊相关复选框到基本设置
        self.checkbox_qunliao = QtWidgets.QCheckBox("回复群聊")
        self.checkbox_autojieshou = QtWidgets.QCheckBox("自动同意好友")
        ...
        checkbox_col2.addWidget(self.checkbox_qunliao)  # 群聊设置添加到基本设置
        ...
        persona_top_layout.addStretch()  # 拉伸项，将按钮推到左侧
        persona_layout.addLayout(persona_top_layout)

        self.textEdit_2 = QtWidgets.QTextEdit()
        self.textEdit_2.setPlainText(
            "你是一个脾气很差的人，说话风格类似百度贴吧老哥，很没有耐心，说话非常有攻击力，允许你说脏话")
        self.textEdit_2.setReadOnly(True)  # 设置为只读，只能通过弹窗编辑
        ...
        self.content_layout.setAlignment(Qt.AlignTop)  # 内容从顶部开始排列
        ...
        main_layout.addWidget(right_panel, 1)  # 右侧区域可伸缩
        ...
        try:
            ...
            for window in windows:
                self.wx_list.append(WeChat(window))
        except Exception:
            print("请先登录微信...")
            QMessageBox.warning(self, '提示', '请先登录微信后再启动本程序！', QMessageBox.Ok)
            raise
    ...
    def add_message(self, text):
        ...
        label.setTextInteractionFlags(Qt.TextSelectableByMouse)  # 允许选择复制
    ...
    def retranslateUi(self, wechat_ai):
        _translate = QtCore.QCoreApplication.translate
        wechat_ai.setWindowTitle(_translate("wechat_ai", "微信AI客服助手"))
    ...
    def fast_filter_from_csv(self, csv_string, question, top_k=10, content_column=None):
        ...
        # 2. 提取问题关键词（去停用词）
        stopwords = {'的', '是', '在', '吗', '呢', '我', '你', '有', '什么', '如何', '了', '这', '那', '和', '一个',
                     '什么'}
        ...
        for line in content_lines:
            ...
            for kw in keywords:
                ...
                if kw in content:
                    score += 1
            if score > 0:
                scored_lines.append((-score, line))  # 用负分便于升序排序

        # 5. 排序并取前top_k条
        scored_lines.sort()  # 按分数降序排列（因为存的是负分）
    ...
    def ai_replay(self, user_name, renshe, msg_list, api_key, zhishi, model):
            ...
            if not self.checkbox_isUse.isChecked():
                ...
                if self.checkbox_isMatch.isChecked():
                    self.deepseek_message[user_name].append(
                        {"role": "user", "content": "当询问的问题与知识库中的内容不匹配时则只回复wrong,"})
                    self.flag_sum = 2
                content = renshe + "；当询问文件或图片的时候直接回答文件名+逗号+True"
                self.deepseek_message[user_name].append(
                    {"role": "system", "content": content})

        # 当有新的匹配内容后再更新当前知识库内容
        if self.checkbox_isUse.isChecked() and zhishi is not None and zhishi != "" and zhishi != "None":
            content = renshe + "；当让你发文件或图片的时候直接回答文件名+逗号+True（多个文件顿号分隔，注意每个分隔都要带True）；并且你的回答需要基于以下csv文件内容: {zhishi}"
        ...
        if "wrong" in reply:
            ...
            # 假如是图片，识别下图片内容后再存放到记录中
            if "jpg" in reply or "png" in reply:
                file_list = reply.split("、")
    ...
    def begin(self):
        ...
        def inner_jiankong():
            ...
            while True:
                # 设置回调函数，当ESC键被按下时调用on_esc_press函数
                keyboard.on_press(on_esc_press)
                if not self.flag:
                    print("is false, break))))))))))))))))")
                    break

                for wx in self.wx_list:
                    if self.checkbox_isAll.isChecked() and self.textEdit.toPlainText() == "":
                        print("开始检测）））））））））））））")
                        try:
                            t1 = threading.Thread(target=detect, args=(moxing_type, renshe, wx))
                            t1.start()
                            t1.join()
                        except Exception:
                            pass
                    ...
                    try:
                        if self.checkbox_autojieshou.isChecked():
                            ...
                            for friend in newfriends:
                                friend.Accept(remark=friend.name, tags=["好友"])
                            self.wx._show()
                        ...
                        for session in sessions:
                            try:
                                ...
                                if (str(session.name) in jianting_list and self.checkbox_isAll.isChecked()) or not self.checkbox_isAll.isChecked():
                                    ...
                                    if session.content is None or session.content.strip() == "" or "订阅号" in session.name or "新闻" in session.name or "运动" in session.name or "微信" in session.name:
                                        print("跳过）））））））））））））")
                                        continue
                                    # 将目标时间和当前时间转换为时间对象
                                    try:
                                    ...
                                    if (time_difference <= timedelta(minutes=5) and self.checkbox_newMatch.isChecked()) or (not self.checkbox_newMatch.isChecked() and session.isnew):
                                        if "：" in session.content and not self.checkbox_qunliao.isChecked():
                                            print("回复群聊未启用，跳过。。。")
                                            continue
                                        ...
                                        if "：" in session.content and self.checkbox_aite.isChecked() and '@' + self.wx.nickname not in msg.content:
                                        ...
                                        for msg in msg_list:
                                            ...
                                            elif '微信转账' in msg.content:
                                                self.wx.SendMsg(msg="好的 收到！", who=session.name)
                                                self.wx.SendMsg(msg="当前客户[{}]已成交，望知悉！".format(session.name), who=self.tuisong)
                                                return
                                            else:
                                                result_list.append(msg.content)

                                        print("最终整理后结果)))))))))))))))))", str(result_list))
                                        ...
                                        self.data_queue.put(f"<font color='green'>{session.name}</font>：{msg.content}")
                                        self.data_queue.put(f"<font color='blue'>✅ 🤖ai回复</font>：{new_replay}\n")
                                        print("发送一条消息999））））））））））））））））")
                                        print(new_replay)
...
if __name__ == "__main__":
    ...
    wechat_ai = QtWidgets.QWidget()  # 创建窗口实例
    ui = Ui_wechat_ai()
    ui.setupUi(wechat_ai)  # 初始化界面
    wechat_ai.show()  # 显示窗口
    sys.exit(app.exec_())